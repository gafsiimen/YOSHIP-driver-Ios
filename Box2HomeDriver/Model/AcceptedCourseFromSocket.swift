...
//
//   let welcome = try? newJSONDecoder().decode(AcceptedCourse.self, from: jsonData)

import Foundation

struct AcceptedCourse: Codable {
    let id: Int
    let code, courseSource: String
    let adresseDepart: Adresse
    let pointEnlevement: JSONNull?
    let vehiculeType: String
    let moyenPaiement: MoyenPaiement
    let observation: String
    let observationArrivee: JSONNull?
    let factures: String
    let adresseArrivee: Adresse
    let chauffeur: Chauffeur
    let vehicule: Vehicule
    let lettreDeVoiture: LettreDeVoiture
    let contactArrivee, contactDepart: JSONNull?
    let nombreColis: Int
    let manutention, manutentionDouble: Bool
    let estimatedKM: Double
    let status: Status
    let commande: Commande
    let dateDemarrage, dateAcceptation: Date
    let dateEnlevement, dateLivraison, dateAffirmationFin: JSONNull?
    let createdAt: Date
    let montantHT: String
    let signaturesImages, colisImages, scannedDocs, articles: [JSONAny]
    let articleFamilies: [ArticleFamily]
    let isStatusChangedManually: Bool
    let dateDemarrageMeta: JSONNull?
...
struct Adresse: Codable {
    ...
    let postalCode: Int
    let operationalHours: [JSONAny]
}

struct ArticleFamily: Codable {
    let articleFamily: MoyenPaiement
}
...
struct Chauffeur: Codable {
    let lastname, firstname, code: String
}

struct Commande: Codable {
    let canalVente: CanalVente
    let client: Client
    let courses: [JSONNull?]
    let promotion, commandeExtra: JSONNull?
    let montantGlobalHT, codeTva, montantGlobalTtc: Int
    let etatPaiement: String
    let collaborateur: Collaborateur
    
    enum CodingKeys: String, CodingKey {
        case canalVente, client, courses, promotion, commandeExtra
        case montantGlobalHT = "montantGlobalHt"
        case codeTva, montantGlobalTtc, etatPaiement, collaborateur
    }
}
...
struct CanalVente: Codable {
    ...
    let articleFamilies: [MoyenPaiement]
}

struct Configs: Codable {
    let priceBasedOnPurchaseAmount, priceBasedOnNBitems, fixedPriceIncludeManutention: Bool
    let operationalHours: [OperationalHour]
}

struct OperationalHour: Codable {
    let dayOfWeek: MoyenPaiement
...
struct Client: Codable {
    ...
    let avatarURL, societe: JSONNull?
}
...
struct Vehicule: Codable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case haillon, denomination, immatriculation
    }
}
